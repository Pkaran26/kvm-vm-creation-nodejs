...
// --- Configuration ---
const VM_NAME = 'ubuntu-noble-vm-1';
const OS_IMAGE_URL = 'https://cloud-images.ubuntu.com/noble/current/noble-server-cloudimg-amd64.img';
const DOWNLOAD_DIR = './vm_images'; // Directory to store downloaded and created images
const BASE_IMAGE_NAME = path.basename(OS_IMAGE_URL);
const VM_DISK_NAME = `${VM_NAME}.qcow2`;
const CLOUD_INIT_ISO_NAME = `${VM_NAME}-cloud-init.iso`;

// VM Specs
const VM_MEMORY = '3072'; // MB
const VM_VCPUS = '2';
const VM_DISK_SIZE = '20G'; // Size for the new VM disk based on the cloud image

// Cloud-init User Data
// IMPORTANT: Replace with your actual public SSH key
const SSH_PUBLIC_KEY = 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIS0mmbs3DZBIiJRvxdOQYT0L44+IButThC+egyUBU/+ Indiqus';
...
async function createKvmVm() {
    try {
        ...
        if (!fs.existsSync(baseImagePath)) {
            ...
            console.log(`[INFO] Base image ${baseImagePath} already exists. Skipping download.`);
        }
        ...
        if (fs.existsSync(vmDiskPath)) {
            console.warn(`[WARN] VM disk ${vmDiskPath} already exists. Deleting and recreating.`);
            fs.unlinkSync(vmDiskPath);
        }
        await runCommand(
          `qemu-img create -f qcow2 -b ${path.basename(baseImagePath)} -F qcow2 ${vmDiskPath} ${VM_DISK_SIZE}`,
...
`;
        ...
        if (fs.existsSync(cloudInitIsoPath)) {
            console.warn(`[WARN] Cloud-init ISO ${cloudInitIsoPath} already exists. Deleting and recreating.`);
        ...
        try {
            ...
            try {
                ...
                console.error('[ERROR] Neither genisoimage nor mkisofs found. Please install one of them.');
                throw e2;
            }
        }
        ...
        console.log('[INFO] Ensure you have permissions to run virt-install (often requires sudo).');
...
sudo virt-install \\
    ...
    --network network=myfreenetwork,model=virtio \\
    --import \\
    --noautoconsole`;
        ...
        console.log(`2. If running, find its IP address: sudo virsh domifaddr ${VM_NAME}`);
        console.log(`   (Cloud-init might take a minute or two to apply network settings and get an IP)`);
        console.log(`3. Connect via SSH: ssh ${USERNAME}@<VM_IP_ADDRESS> (using the SSH key you provided)`);
        console.log(`4. To access the console/display: Use a VNC client to connect to your host's IP and the VNC port assigned by libvirt (check with 'sudo virsh vncdisplay ${VM_NAME}')`);
        console.log(`5. To manage the VM: sudo virsh <command> ${VM_NAME} (e.g., shutdown, start, destroy)`);
...
// --- Run the script ---
// if (SSH_PUBLIC_KEY.includes('your_email@example.com')) {
//     console.error('[ERROR] Please replace the placeholder SSH_PUBLIC_KEY in the script with your actual public SSH key.');
// } else {
    createKvmVm();
// }